export default async function handler(req,res){
  ...
  try{
    ...
    const lineItems=(payload.items||[]).map(item=>{
      if(item.variantId){ return {variant_id:Number(item.variantId), quantity:Number(item.quantity)||1}; }
      else{ return {title:item.title||"Custom Product", price:Number(item.price)||0, quantity:Number(item.quantity)||1}; }
    });
    ...
    const shopifyResponse=await fetch("https://b007a7-f0.myshopify.com/admin/api/2025-07/orders.json",{
      method:"POST",
    ...
    if(!shopifyResponse.ok){
      ...
      return res.status(shopifyResponse.status).json({error:"Failed to create Shopify order",details:errorText});
    }

    const shopifyData=await shopifyResponse.json();
    console.log("✅ Shopify order created:",shopifyData);
    return res.status(200).json({success:true,order:shopifyData});
  }catch(err){ console.error("🔥 Webhook error:",err); return res.status(500).json({error:"Internal Server Error"}); }
        }
